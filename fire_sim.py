...
st.title("🔥 Simulatore FIRE: Dashboard Completa")
st.write("Calcola il tuo percorso verso l'indipendenza finanziaria, includendo bonus e imprevisti positivi.")

# --- 1. IL "MOTORE" DEL SIMULATORE (Aggiornato con il Bonus) ---
def simula_monte_carlo(p_iniziale, risparmio, spese, rend_nom, infl, tasse, swr, vol, n_sim, anni_totali, bonus_euro, bonus_anno):
    target = spese / swr
    ...
    for i in range(n_sim):
        ...
        for mese in range(mesi_totali):
            ...
            # CONTROLLO EVENTO EXTRA: Inseriamo il bonus nel mese esatto
            # Usiamo -1 perché l'indice dei mesi parte da 0
...
# NUOVA SEZIONE: Evento Straordinario
st.sidebar.header("🎁 Evento Straordinario")
importo_bonus = st.sidebar.number_input("Importo (Eredità/TFR/Bonus) (€)", value=0, step=5000)
...
df_sim, capitale_target = simula_monte_carlo(
    p_iniziale, r_mensile, spese_annuali, rend_nom, inflazione, tasse_cg, swr, volatilita, n_simulazioni, anni_restanti, importo_bonus, anno_del_bonus
)
...
if mese_fire:
    ...
    if importo_bonus > 0:
        st.info(f"💡 Il bonus di {importo_bonus} € previsto tra {anno_del_bonus} anni è stato calcolato nel percorso!")
else:
    st.warning("⚠️ Con i parametri attuali, non raggiungerai il target FIRE entro l'aspettativa di vita. Prova ad aumentare il risparmio mensile.")
